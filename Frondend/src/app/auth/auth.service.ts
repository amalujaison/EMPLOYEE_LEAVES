...
export class AuthService{
    ...
    autologin(){
        ...
        const loadedUser = new User(userData.user_id,userData.email,userData.username,userData.name,userData.role,userData.token)
    ...
    private handleAuth(res: AuthResData){
        const user = new User(res.user_id,res.email,res.username,res.name,res.role,res.token);
        this.user.next(user);
